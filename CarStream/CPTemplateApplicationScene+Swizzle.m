...
static void CarStreamSwizzleInstanceMethod(Class cls, SEL originalSEL, SEL replacementSEL) {
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        CarStreamSwizzleInstanceMethod(cls,
...
+ (void)load {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        CarStreamSwizzleInstanceMethod(self,
...
static void CarStreamWrapBoolNoArg(Class cls, SEL sel) {
...
static void CarStreamWrapObjectNoArg(Class cls, SEL sel) {
    Method method = class_getInstanceMethod(cls, sel);
    if (!method) {
        NSLog(@"[CarPlayTrace] missing %@ on %@", NSStringFromSelector(sel), cls);
        return;
...
static void CarStreamWrapVoidOneObjectArg(Class cls, SEL sel) {
    Method method = class_getInstanceMethod(cls, sel);
    if (!method) {
        NSLog(@"[CarPlayTrace] missing %@ on %@", NSStringFromSelector(sel), cls);
        return;
...
static void CarStreamInstallCarPlayTrace(void) {
    Class env = NSClassFromString(@"CPSTemplateEnvironment");
    if (env) {
        CarStreamWrapBoolNoArg(env, NSSelectorFromString(@"hasAnyTemplateEntitlement"));
        CarStreamWrapBoolNoArg(env, NSSelectorFromString(@"hasNavigationEntitlement"));
        CarStreamWrapBoolNoArg(env, NSSelectorFromString(@"hasCommunicationEntitlement"));
        CarStreamWrapBoolNoArg(env, NSSelectorFromString(@"hasAudioEntitlement"));
        CarStreamWrapBoolNoArg(env, NSSelectorFromString(@"isUserApplication"));
        CarStreamWrapObjectNoArg(env, NSSelectorFromString(@"allowedTemplateClasses"));
    } else {
    ...
    if (instance) {
        CarStreamWrapBoolNoArg(instance, NSSelectorFromString(@"clientApplicationSceneIsConnected"));
        CarStreamWrapBoolNoArg(instance, NSSelectorFromString(@"requiresApplicationScenePresenter"));
        CarStreamWrapVoidOneObjectArg(instance, NSSelectorFromString(@"setWindowSceneForTemplateApplicationScene:"));
    } else {
